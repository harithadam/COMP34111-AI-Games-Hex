...
class MinimaxBeamAgent(AgentBase):
    ...
    def _get_canonical_key(self, board: Board) -> tuple:
        ...
        # Reverse rows, then reverse elements in each row
    ...
    def _minimax(
        ...
        if state_key in self.tt:
            entry = self.tt[state_key]
            # We can only use the cached value if the cached depth is >= current depth
    ...
    def _order_moves(
        ...
        for move in moves:
            ...
            # 2. Adjacency / Bridge detection (Fast)
            adj = 0
            ...
            # Use ONLY these fast metrics for sorting
            # We removed the expensive self._evaluate(child) call!
            score = 2 * centrality + 3 * adj
